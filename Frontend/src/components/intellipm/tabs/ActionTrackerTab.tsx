...
export default function ActionTrackerTab({ actions: initialActions, tracker }: Props) {
  ...
  const thClass = 'px-3 py-3 text-left text-xs font-semibold text-slate-500 uppercase tracking-wider bg-slate-50 cursor-pointer hover:bg-slate-100 select-none focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-indigo-500';
  ...
  return (
    <div className="space-y-5">
      ...
      >
        {[
          ...
          },
          {
            ...
            color: tracker.overdue_actions > 0 ? 'text-red-700' : 'text-emerald-700',
            bg: tracker.overdue_actions > 0 ? 'bg-red-50 border-red-200' : 'bg-emerald-50 border-emerald-200',
            iconBg: tracker.overdue_actions > 0 ? 'bg-red-100 text-red-600' : 'bg-emerald-100 text-emerald-600',
          },
          {
            label: 'Avg Age (days)',
      ...
      >
        <h3 id="owner-dist-heading" className="text-sm font-bold text-slate-700 mb-3">
          Actions by Owner
        </h3>
            ...
            .map(([owner, count]) => {
              ...
              return (
                <div key={owner} className="flex items-center gap-3">
                  <span className="text-xs text-slate-600 w-28 truncate flex-shrink-0">{owner}</span>
                  <div
                  ...
                  <span className="text-xs font-semibold text-slate-700 w-6 text-right flex-shrink-0">
                    {count}
                  </span>
                </div>
              );
      ...
      <div className="flex flex-wrap gap-3 items-center justify-between">
        <div className="flex flex-wrap gap-3 items-center" role="group" aria-label="Action filters">
          <div>
            <label htmlFor="filter-status" className="text-xs font-semibold text-slate-600 mr-1.5">
              Status:
            </label>
            <select
              id="filter-status"
          ...
          <div>
            <label htmlFor="filter-owner" className="text-xs font-semibold text-slate-600 mr-1.5">
              Owner:
            </label>
            <select
              id="filter-owner"
        ...
        <button
          ...
          className="flex items-center gap-2 text-xs bg-indigo-600 text-white px-3 py-1.5 rounded-lg hover:bg-indigo-700 focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-indigo-500 focus-visible:ring-offset-1 transition-colors font-medium"
              ...
              sorted.map((action, i) => {
                ...
                return (
                  ...
                  >
                      ...
                      {isEditing ? (
                        <div className="flex gap-2 items-center">
                          <input
                            ...
                            className="text-xs border border-indigo-400 rounded px-2 py-1 w-full focus:outline-none focus:ring-2 focus:ring-indigo-500"
                            aria-label="Edit action description"
                            autoFocus
                            onKeyDown={(e) => { if (e.key === 'Enter') saveEdit(globalIdx); if (e.key === 'Escape') setEditIdx(null); }}
                          />
                          <button onClick={() => saveEdit(globalIdx)} className="text-xs bg-indigo-600 text-white px-2 py-1 rounded whitespace-nowrap focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-indigo-500">Save</button>
                          <button onClick={() => setEditIdx(null)} className="text-xs bg-slate-200 px-2 py-1 rounded whitespace-nowrap focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-indigo-500">Cancel</button>
                        </div>
                      ) : (
                    ...
                    <td className="px-3 py-3 text-slate-700 whitespace-nowrap text-xs">{action.owner}</td>
                    <td className="px-3 py-3 text-slate-700 whitespace-nowrap text-xs font-mono">{action.due_date}</td>
                    <td className="px-3 py-3"><HealthBadge status={action.status} size="sm" /></td>
                    <td className="px-3 py-3">
                      <span className={`text-xs font-semibold ${action.age_days >= 10 ? 'text-red-700' : action.age_days >= 5 ? 'text-amber-700' : 'text-slate-700'}`}>
                        {action.age_days}d
                      </span>
                    </td>
